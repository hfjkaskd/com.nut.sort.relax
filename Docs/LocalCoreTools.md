...
Insufficient inventory opens the existing local SDK-skipped notice. No-history and maximum-add now use the source nonmodal TopCanvas Tip prefab/text and one-second lifetime; see LocalPopTips.md. No ad, reward, cash or inventory grant is fabricated. Tests supply explicitly identified inventory fixtures only.

Validation: LocalToolsPlayValidation runs the actual default scene, standard Button listeners and camera-ray gameplay. It covers a normal transfer, undo cost/history and reverse-animation board restoration, exchange mode/slot rotation/save/display/cancel delay, add-rod unlock and cost/save, nonmodal no-history toast and insufficient-stock notice, modal-mask expiry, and the actual bottom replay popup. No runtime callbacks or initialization flags are substituted. This tool test feeds the real ray-input method and Button listeners explicitly. ExchangeTapInput.md records a later regression using actual UI raycast/Button dispatch in both release orders; target-device hardware input still needs verification.
