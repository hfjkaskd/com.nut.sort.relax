...
Remaining differences: ScenePosGroup still represents row positions in the existing flattened hierarchy instead of the original row-parent hierarchy. Source sibling-order equivalence is verified for ordinary initialized/saved row ordering only. Default startup still needs real configuration/panel context and tool-flow composition; this branch does not invent SDK or server responses. Full production startup, lifecycle and visual parity remain incomplete.
