...
Global callback 0x9E3D98 ignores its payload, calls ShowEveryDayGift, invokes the main panel's RefreshLssPanel (virtual slot 7), then reads the panel's current ShowBanner flag. When true it calls TargetRewardBanner.Show with a non-null empty completion callback (0x9E4D98). This flag is read after refresh, including re-entry effects, rather than captured when the callback is bound.
...
Correction: the exported ARM64 layout places the vtable at 0x138. Dispatch at 0x1A8 selects ordinary RefreshLssPanel, not the empty base TweenEndRefreshPanel.
