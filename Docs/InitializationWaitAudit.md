...
Validation checks lazy first yield, null/inactive/destroyed/replacement object semantics, the second scaled-time yield and one-shot optional continuation. This restores the waiting stage only. It is not yet automatically invoked by full InitLevel and does not skip the subsequent state-dependent sync/InitDone branches. SDK handling is unchanged.
