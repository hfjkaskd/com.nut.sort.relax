...
回归覆盖注册/配置分支、保留请求回调、同步返回、两种布尔值、配置与用户替换、空白触发、空配置、初始化/请求异常及空奖励回调。请求与 ServerConfigData.Init 均为必需注入端口，没有新增 SDK 调用或自动响应。正式初始化尚未组合到主场景，配置 Init 非 SDK 本体已随后恢复并完成实际组合回归，详见 ServerConfigInitializationAudit.md；本轮不能证明启动生命周期完成。
