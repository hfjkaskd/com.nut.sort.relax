...
Play 集成夹具使用真实 23/33 Prefab 和宿主；时间工具已随后恢复并用于当前 Play：使用本机墙上时间与原版地区格式化（详见 WithdrawalTimeAudit.md）。地区、奖励数据与最终 UI18 目的地仍是明确测试边界。正式初始化与主流程路由尚未接通，本轮不构成全生命周期完成证据。SDK 处理保持原样。
