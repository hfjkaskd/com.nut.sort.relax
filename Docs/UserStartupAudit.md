...
集成测试覆盖新用户顺序、两次独立时钟、发布时机、姓名/标识失败、已有存档不调用新用户步骤、实际配置完成回调与保存、null 和损坏存档。后续已通过 OriginalUserSession.StartUser 接入实际会话宿主，并完成场景/音频共享新 Store 的 Play 验证，详见 UserSessionStartupAudit.md。自动启动调用方和 SDK 输入仍未接通，不声明玩家完整启动链路完成。
