...
Modal input covers both the local result/notice and the native unlock host. The original counted click-mask predicate includes this host so expiry cannot enable world input beneath an open popup. Closing waits for registry removal and accounts for pending click masks/replay before restoring input. The source IsInitDone clock state remains true while the unlock modal is shown.
