...
- Original deadlock/failure delay reaches the local retry panel. Its Button uses RestartAfterFailure and the original seed increment. Existing replay uses the original prefab/controller. Captured-board guards prevent delayed local callbacks from acting on replacement boards.
