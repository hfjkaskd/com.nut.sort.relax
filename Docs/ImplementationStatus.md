...
- 已恢复 12 色螺母、隐藏螺母和槽位 Prefab，接入原生显示组件与延迟加载对象池。隐藏/揭示、对象复用及槽位显示规则通过实例验证。灰度 Shader 的 12 组 GPU 像素测试通过，完整回归仍通过。原始提取证据保留本机；入场、选中、放回和摆动动画已随后实现；跨杆搬移、完成特效和主场景绑定仍未完成。

- 已在原螺母 Prefab 接入原生入场、抬起、放回和选中摆动动画，参数及缓动曲线由配置资源驱动。原生 Transform 采样验证覆盖关键位置、旋转方向、延迟、回调顺序、中断及池化释放，完整回归通过。跨杆移动、落点火花实际播放、整杆完成动画和主场景绑定仍待接入，尚未完成动态画面对照。
