...
namespace NutSort.Validation
{
    ...
    public static class OriginalWithdrawalStageZeroPlayValidation
    {
        ...
        static OriginalWithdrawalStageZeroPlayValidation(){if(SessionState.GetBool(Key,false)){timeout=EditorApplication.timeSinceStartup+300;EditorApplication.update+=Tick;}}
        ...
        private static void Tick()
        {
            ...
            try
            {
                Check(EditorApplication.timeSinceStartup<timeout,"Stage-zero Play timeout phase="+phase+" scaledTime="+Time.time+" phaseStart="+start);
                if(phase==0)
                {
                ...
                Debug.Log("NUT_WITHDRAWAL_STAGE_ZERO_PLAY_PASS actual stage23 first animation to hint33 and Button return to actual stage23 resume, final Sure UI18 route and animated removal; native live local clock and country-formatted dates; data/country/UI18 destination are explicit fixtures, no production/SDK claim.");Finish(0);
            }
